# pyright: reportMissingImports=false, reportUnknownVariableType=false, reportUnknownParameterType=false, reportUnknownMemberType=false, reportUnknownArgumentType=false, reportUnusedCallResult=false, reportImplicitStringConcatenation=false
...
from terminus2_support.core_agent import (
    AgentCallbacks,
    Command,
    ...
    *,
    ...
    for prefix in ("New Terminal Output:\n", "Current Terminal Screen:\n"):
        if raw.startswith(prefix):
            raw = raw[len(prefix) :]
            break
    output_text = raw.strip() if raw.strip() else "[no output]"
    if verbosity == 0:
        in_prefix = "in: "
        out_prefix = "out: "
