# pyright: reportMissingImports=false, reportAny=false, reportExplicitAny=false, reportUnknownVariableType=false, reportUnknownMemberType=false, reportUnknownParameterType=false, reportUnknownArgumentType=false, reportUnusedCallResult=false
...
from liteforge_support.agent import (
    build_system_prompt,
    build_user_prompt,
    get_environment,
    list_cwd_files,
)
from liteforge_support.context import Context
from liteforge_support.logging_utils import last_assistant_text
from liteforge_support.orchestrator import Orchestrator
from liteforge_support.tools.executor import ToolExecutor
from liteforge_support.tools.registry import (
    ALL_TOOL_NAMES,
    READONLY_TOOL_NAMES,
    build_tool_definitions,
)
from workspace_agent_types import WorkspaceRunResult, WorkspaceRuntimeConfig
...
def _resolve_tool_names(*, options: dict[str, Any]) -> list[str]:
    ...
    if isinstance(configured, list):
        ...
        if tool_names:
            return tool_names
    if bool(options.get("readonly", False)):
        ...
        *,
        ...
        try:
            with _temporary_environ(overrides=env_overrides):
                ...
                if hasattr(orchestrator, "set_log_console"):
                    orchestrator.set_log_console(console=console)
