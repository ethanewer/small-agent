...
def discover_repo_root(*, start_path: Path) -> Path:
    ...
    for candidate in (current, *current.parents):
        config_dir = candidate / "agent_evolve_v2"
        if any((config_dir / name).exists() for name in RUNS_CONFIG_NAMES):
            return candidate
    raise FileNotFoundError(
        f"Unable to locate repo root from {start_path} via agent_evolve_v2/runs.json."
    )
...
def stage_remote_bundle(
    *,
    ...
    staging = Path(tempfile.mkdtemp(prefix="agent-evolve-v2-bundle-"))
    shutil.copytree(src=agent_dir, dst=staging / "agent", symlinks=True)
    shutil.copy2(
        src=repo_root / "agent_evolve_v2" / "service_remote_runner.py",
